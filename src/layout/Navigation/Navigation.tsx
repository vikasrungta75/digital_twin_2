...
const Navigation = forwardRef<HTMLElement, INavigationProps>(
	({ menu, horizontal, id, className, ...props }, ref) => {
		...
		const permissions : any = useSelector((state: RootState) => state.auth?.permissions);

		const hideNavItem = (item: string, hide: boolean) => {
			...
			return Object.keys(data).map((item) =>
				data[item].path ? (
					<Item
						...
						hide={hideNavItem(data[item].id, data[item].hide)}>
						{!!data[item].subMenu &&
							fillMenu(
								data[item].subMenu,
								data[item].id,
