...
const useSelectTable = (data: any[], uniqueId: string) => {
	...
	useEffect(() => {
		if (selectTable.values.selectAll) {
			selectTable.setValues({
				...selectTable.values,
				// @ts-ignore
				selectedList: data.map((d) => d?.[uniqueId]?.toString()),
			});
		} else {
			selectTable.setValues({
				...selectTable.values,
	...
	}, [selectTable.values.selectAll, data]);
	...
	useEffect(() => {
		if (ref.current) {
			if (
				...
				selectTable.values.selectedList.length !== data.map((d) => d?.[uniqueId]?.toString()).length
			) {
