...
const Select = forwardRef<HTMLSelectElement, ISelectProps>(
	(
		...
		return (
			<div className={`${isLoading ? 'd-flex align-items-center flex-column position-relative' : ''}`}>
				<select
					ref={ref}
					id={id}
				...
				{isLoading && (
					<div
						style={{
							...
							right: 12,
							top:12,
							marginRight: dir === 'rtl' ? -32 : 0,
							marginLeft: -32,
							// marginTop: 2,
							background: disabled ? '#e9ecef' : '#f8f9fa',
						}}>
						<Spinner size='20px' color='secondary' />
					</div>
				)}
				{isValidMessage && (
					<Validation
